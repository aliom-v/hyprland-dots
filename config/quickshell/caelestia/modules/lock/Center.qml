        ...
        StyledText {
            ...
            readonly property string msg: {
                ...
                if (pam.state === "max" && pam.fprintState === "max")
                    return qsTr("密码和指纹尝试次数都已达上限。");
                if (pam.state === "max") {
                    if (pam.fprint.available)
                        return qsTr("密码尝试次数已达上限，请改用指纹。");
                    return qsTr("密码尝试次数已达上限。");
                }
                if (pam.fprintState === "max")
                    return qsTr("指纹尝试次数已达上限，请改用密码。");

                if (pam.state === "fail") {
                    if (pam.fprint.available)
                        return qsTr("密码错误，请重试或改用指纹。");
                    return qsTr("密码错误，请重试。");
                }
                if (pam.fprintState === "fail")
                    return qsTr("指纹未识别（%1/%2），请重试或改用密码。").arg(pam.fprint.tries).arg(Config.lock.maxFprintTries);
