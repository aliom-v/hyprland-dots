...
Column {
    ...
    StyledText {
        ...
        text: UPower.displayDevice.isLaptopBattery ? (UPower.onBattery ? qsTr("剩余时间：%1").arg(formatSeconds(UPower.displayDevice.timeToEmpty, "计算中...")) : qsTr("距充满：%1").arg(formatSeconds(UPower.displayDevice.timeToFull, "已充满"))) : qsTr("电源方案：%1").arg(PowerProfile.toString(PowerProfiles.profile))
    }
