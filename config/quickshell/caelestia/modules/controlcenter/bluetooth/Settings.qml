...
ColumnLayout {
    ...
    function adapterStateText(): string {
        ...
        const stateText = BluetoothAdapterState.toString(adapter.state);
        const lowered = String(stateText).toLowerCase();
        if (lowered.includes("turning") && lowered.includes("on"))
            return qsTr("正在开启");
        if (lowered.includes("turning") && lowered.includes("off"))
            return qsTr("正在关闭");
        if (lowered.includes("off") || lowered.includes("disabled"))
            return qsTr("已关闭");
        if (lowered.includes("on") || lowered.includes("enabled"))
            return qsTr("已开启");
        return stateText;
    }
