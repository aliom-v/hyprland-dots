...
StyledFlickable {
    ...
    function fabActionLabel(actionName: string): string {
        switch (actionName) {
        case "connect":
            return root.device?.connected ? qsTr("断开连接") : qsTr("连接");
        case "trust":
            return root.device?.trusted ? qsTr("取消信任") : qsTr("设为信任");
        case "block":
            return root.device?.blocked ? qsTr("取消屏蔽") : qsTr("屏蔽设备");
        case "pair":
            return root.device?.paired ? qsTr("取消配对") : qsTr("配对设备");
        default:
            return actionName;
        }
    }
                    ...
                    ColumnLayout {
                        ...
                        StyledRect {
                            ...
                            ColumnLayout {
                                ...
                                StyledText {
                                    text: root.device?.batteryAvailable ? qsTr("设备电量（%1%）").arg(Math.round(root.device.battery * 100)) : qsTr("电量不可用")
                                }
        ...
        Repeater {
            ...
            StyledClippingRect {
                ...
                RowLayout {
                    ...
                    StyledText {
                        animate: true
                        text: root.fabActionLabel(fabMenuItem.modelData.name)
                        color: Colours.palette.m3onPrimaryContainer
