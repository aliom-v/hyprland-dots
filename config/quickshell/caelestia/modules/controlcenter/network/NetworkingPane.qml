    ...
    SplitPaneLayout {
            ...
            StyledFlickable {
                ...
                ColumnLayout {
                    ...
                    RowLayout {
                        ...
                        ToggleButton {
                            toggled: root.session.network.showSettings || (!root.session.vpn.active && !root.session.ethernet.active && !root.session.network.active)
                            icon: "settings"
                            accent: "Primary"
                            iconSize: Appearance.font.size.normal
                            horizontalPadding: Appearance.padding.normal
                            ...
                            onClicked: {
                                root.session.network.showSettings = true;
                            }
                        }
                    }
        ...
        rightContent: Component {
            Item {
                ...
                property bool displaySettings: root.session.network.showSettings || !pane
                property string paneId: displaySettings ? "settings" : (vpnPane ? ("vpn:" + (vpnPane.name || "")) : (ethernetPane ? ("eth:" + (ethernetPane.interface || "")) : (wirelessPane ? ("wifi:" + (wirelessPane.ssid || wirelessPane.bssid || "")) : "settings")))
                property Component targetComponent: settingsComponent
                property Component nextComponent: settingsComponent

                function getComponentForPane() {
                    if (displaySettings || !pane)
                ...
                Connections {
                    ...
                    function onActiveChanged() {
                        ...
                        if (root.session && root.session.vpn && root.session.vpn.active) {
                            root.session.network.showSettings = false;
