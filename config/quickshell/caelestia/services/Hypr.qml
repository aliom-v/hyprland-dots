...
Singleton {
    ...
    readonly property string kbLayoutFull: keyboard?.activeKeymap ?? qsTr("未知")
    ...
    onKbLayoutFullChanged: {
        if (hadKeyboard && Config.utilities.toasts.kbLayoutChanged)
            Toaster.toast(qsTr("键盘布局已切换"), qsTr("布局已切换为：%1").arg(kbLayoutFull), "keyboard");
