...
Singleton {
    ...
    function getStreamName(stream: PwNode): string {
        ...
        return stream.applicationName || stream.description || stream.name || qsTr("未知应用");
    }
    ...
    onSinkChanged: {
        ...
        if (previousSinkName && previousSinkName !== newSinkName && Config.utilities.toasts.audioOutputChanged)
            Toaster.toast(qsTr("音频输出已切换"), qsTr("当前使用：%1").arg(newSinkName), "volume_up");
    ...
    onSourceChanged: {
        ...
        if (previousSourceName && previousSourceName !== newSourceName && Config.utilities.toasts.audioInputChanged)
            Toaster.toast(qsTr("音频输入已切换"), qsTr("当前使用：%1").arg(newSourceName), "mic");
