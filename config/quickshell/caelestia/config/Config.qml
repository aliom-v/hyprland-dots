    ...
    Timer {
        ...
        onTriggered: {
            ...
            try {
                ...
                Toaster.toast(qsTr("配置序列化失败"), e.message, "settings_alert", Toast.Error);
            }
        }
    }
        ...
        onLoaded: {
            try {
                ...
                if (adapter.utilities.toasts.configLoaded && !recentlySaved && elapsed > 0) {
                    Toaster.toast(qsTr("配置已加载"), qsTr("配置已在 %1 毫秒内加载").arg(elapsed), "rule_settings");
                } else if (adapter.utilities.toasts.configLoaded && recentlySaved && elapsed > 0) {
                    Toaster.toast(qsTr("配置已保存"), qsTr("配置已在 %1 毫秒内重新加载").arg(elapsed), "rule_settings");
                }
            } catch (e) {
                Toaster.toast(qsTr("配置加载失败"), e.message, "settings_alert", Toast.Error);
            }
        }
        onLoadFailed: err => {
            if (err !== FileViewError.FileNotFound)
                Toaster.toast(qsTr("读取配置文件失败"), FileViewError.toString(err), "settings_alert", Toast.Warning);
        }
        onSaveFailed: err => Toaster.toast(qsTr("保存配置失败"), FileViewError.toString(err), "settings_alert", Toast.Error)
